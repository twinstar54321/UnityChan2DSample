...
public class UnityChan2DController : MonoBehaviour
{
    ...
    public float unityChanSpeed = 10f;
    public float jumpPower = 1300f;
    public float localGravity = 80f;
    ...
    void Update()
    {
        if (m_state != State.Damaged)
        {
            ...
            Move(x, jump);
            m_rigidbody2D.AddForce(Vector2.down * localGravity);
        }
    }
    ...
    void Move(float move, bool jump)
    {
        ...
        m_rigidbody2D.velocity = new Vector2(move * unityChanSpeed, m_rigidbody2D.velocity.y);
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.tag == "DamageObject" && m_state == State.Normal)
        {
            ...
            if (hitPoint == 0)
            {
                StartCoroutine(INTERNAL_GameOver());
            }
